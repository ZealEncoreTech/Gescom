...
public class BaseService {
	...
	public String getworkRowid(String workDesc){
		String[] args = {workDesc};
		Cursor cursor = null;
		String str ="";
		try{
			baseDao.open();
			cursor = baseDao.Query(DBConstants.TABLE_WORKMAIN, DBConstants.COLUMN_WORKMAIN_WORK_DESCRIPTION +" =? ", args);
			if(cursor != null){
		...
		try {
			...
			if (cursor != null) {
				...
				while (!cursor.isAfterLast()) {
					...
					values.setLtEstId(cursor.getInt(cursor.getColumnIndex(DBConstants.COLUMN_TEMPGPE_LTESTID)));
					values.setCoordinatesCaption(cursor.getString(cursor.getColumnIndex(DBConstants.COLUMN_TEMPGPE_COORDINATES_CAPTION)));
	...
	public double getConductorLengthWithOutPhase(String workRowId,String Typeofline,String materialId,String phase)
	{
		double conductorLegth=0.0;
 		String Query = "select sum(" + DBConstants.COLUMN_GPE_DISTANCE
				+ ") from " + DBConstants.TABLE_GESCOM_PROJECT_ESTIMATES
	...
	public List<ApprovedPhotoCoordinates> getProgressImages(String workRowId,String workStatus,String isImageUploaded){
		List<ApprovedPhotoCoordinates> lstProgressImages = new ArrayList<ApprovedPhotoCoordinates>();
		String[] selArgs = { workRowId,workStatus,isImageUploaded };
		Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_PROGRESS_IMAGES, DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_WORKROWID +"=? and "+DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_WORKSTATUS +"=? and "+DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_ISIMAGEUPLOADED +"=?", selArgs);
		if(cursor!=null){
			while(!cursor.isAfterLast()){
				ApprovedPhotoCoordinates  images = new ApprovedPhotoCoordinates();
				images.setWorkRowId(cursor.getInt(cursor.getColumnIndex(DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_WORKROWID)));
	...
	public long addToProgressImages(ApprovedPhotoCoordinates approvephoto) {
		...
		values.put(DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_ISIMAGEUPLOADED, approvephoto.getIsImageuploaded());
...
public HashMap<String,String> getMultiDescription(String workRowId)
{
    ...
    public boolean workidpresentinphotocoordinates(String workRowId){
    	String[] args = {workRowId};
    	Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_APPROVE_COOR_PHOTOS, DBConstants.COLUMN_GESCOM_APPROVE_COOR_PHOTOS_WorkrowId+ " =? ",args);
    	if(cursor != null){
    		int count = cursor.getCount();
    		if(count >=1){
    			cursor.close();
   ...
   public boolean workidpresentincoordinates(String workRowId){
    	String[] args = {workRowId};
    	Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_APPROVE_COOR, DBConstants.COLUMN_GESCOM_APPROVE_COOR_WORKROWID+ " =? ",args);
    	if(cursor != null){
    		int count = cursor.getCount();
    		if(count >=1){
    			cursor.close();
    ...
    public boolean workidpresentinworkmain(String workRowId){
    	String[] args = {workRowId};
    	Cursor cursor = baseDao.Query(DBConstants.TABLE_WORKMAIN, DBConstants.COLUMN_WORKMAIN_WORKROWID+ " =? ",args);
    	if(cursor != null){
    		int count = cursor.getCount();
    		if(count >=1){
    			cursor.close();
    ...
    public boolean workidpresentinworkItems(String workRowId){
    	String[] args = {workRowId};
    	Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_PROJECT_WORKITEMS, DBConstants.COLUMN_WORKITEMS_WORK_ROW_ID+ " =? ",args);
    	if(cursor != null){
    		int count = cursor.getCount();
    		if(count >=1){
    			cursor.close();
    ...
    public boolean workidpresentinProjectestimates(String workRowId){
    	String[] args = {workRowId};
    	Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_PROJECT_ESTIMATES, DBConstants.COLUMN_GPE_WORK_ROW_ID+ " =? ",args);
    	if(cursor != null){
    		int count = cursor.getCount();
    ...
    public boolean workidpresentinProjectestimateType(String workRowId){
    	String[] args = {workRowId};
    	Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_PROJECT_ESTIMATION_TYPE, DBConstants.COLUMN_GESCOM_PROJECT_ESTIMATIONTYPE_PROJECT_ID+ " =? ",args);
    	if(cursor != null){
    		int count = cursor.getCount();
    ...
    public boolean workrowidinprogressimages(String workrowid){
    	String[] args = {workrowid};
    	Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_PROGRESS_IMAGES, DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_WORKROWID+ " =? ",args);
    	if(cursor != null){
    		int count = cursor.getCount();
    		if(count >= 1){
    			cursor.close();
    ...
    public boolean isWorkRowIdPresentinProgressImages(String projrctId)
    {
    	String[] selargs = { projrctId};
    	
    	
    	Cursor cursor=baseDao.Query(DBConstants.TABLE_GESCOM_PROGRESS_IMAGES, DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_WORKROWID+"=? ", selargs);
	...
	public long insertintoapprovephotocoordinates(ApprovedPhotoCoordinates approvephoto){
		...
		values.put(DBConstants.COLUMN_GESCOM_APPROVE_COOR_PHOTOS_CoordinateId, approvephoto.getCoordinatesId());
   ...
   public List<String> getPhotoPath(String workRowId){
	   String[] selargs = {workRowId};
	   List<String> lst =  new ArrayList<String>();
	   Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_PROGRESS_IMAGES, DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_WORKROWID + "=?" , selargs);
	   if(cursor != null){
		   while(!cursor.isAfterLast()){
			   lst.add(cursor.getString(cursor
						.getColumnIndex(DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_FILENAME)));
			   cursor.moveToNext();
		   }
	   }
	   return lst;
   }
   public List<String> getphotopathfromapprovecoordinates(String workrowid){
	   String[] selargs = {workrowid};
	   List<String> lst = new ArrayList<String>();
	   Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_APPROVE_COOR_PHOTOS, DBConstants.COLUMN_GESCOM_APPROVE_COOR_PHOTOS_WorkrowId + " =? ", selargs);
	   if(cursor != null){
		   while(!cursor.isAfterLast()){
			   lst.add(cursor.getString(cursor.getColumnIndex(DBConstants.COLUMN_GESCOM_APPROVE_COOR_PHOTOS_FileName)));
			   cursor.moveToNext();
		   }
	   }
	   return lst;
	...
	public String getCaptureddate(String projectId){
		String[] selargs = {projectId};
		String cDate = "";
		Cursor cursor = baseDao.Query(DBConstants.TABLE_WORKMAIN,DBConstants.COLUMN_WORKMAIN_WORKROWID + " = ?", selargs);
		if(cursor != null){
			while(!cursor.isAfterLast()){
				cDate = cursor.getString(cursor.getColumnIndex(DBConstants.COLUMN_WORKMAIN_DATES));
	...
	public boolean isUserAvailable(String username, String password) {
		...
		try {
			baseDao.open();
			cursor = baseDao.RawQuery("SELECT * FROM LoginInfo WHERE LoginId LIKE '"+username+"' and Password = '"+password+"'", null);
			/*cursor = baseDao
					.Query(DBConstants.TABLE_AUTH,
							DBConstants.COLUMN_AUTH_USERNAME + "LIKE ?" + " and "
	...
	public String getProjectforSavecordinateforDelete(String staus,String Workrowid) {
		String[] selArgs = { staus,Workrowid };
		String workInfo = "";
		
		Cursor cursor = baseDao.Query(DBConstants.TABLE_WORKMAIN, DBConstants.COLUMN_WORKMAIN_WORKSTATUSID + "=? and "+ DBConstants.COLUMN_WORKMAIN_WORKROWID + " =?  ORDER BY " +DBConstants.COLUMN_WORKMAIN_DATES +" desc ",selArgs);
		/*Cursor cursor = baseDao.Query(DBConstants.TABLE_WORKMAIN,
				DBConstants.COLUMN_WORKMAIN_WORKSTATUSID + "=? and "+ DBConstants.COLUMN_WORKMAIN_ASSIGNED_USER + " =?", selAArgs);*/
		...
		if (cursor != null) {
			...
			while (!cursor.isAfterLast()) {
				int eoid = cursor.getInt(0);
				String eoid1 = cursor.getString(1);
				workInfo = cursor.getString(cursor.getColumnIndex(DBConstants.COLUMN_WORKMAIN_WORK_DESCRIPTION));
				//workInfo.add(cursor.getString(cursor.getColumnIndex(DBConstants.COLUMN_WORKMAIN_WORKROWID)));
				cursor.moveToNext();
			}
	...
	public List<String> getProjectforDeletingImages(String staus,String user) {
		String[] selArgs = { staus,user };
		ArrayList<String> workInfo = new ArrayList<String>();
...
try {
	...
	public long updateworkstatus(int workrowid,String status){
		String[] selargs = {String.valueOf(workrowid)};
		long res ;
		...
		try{
			baseDao.open();
	   res = baseDao.Update(DBConstants.TABLE_WORKMAIN,values,DBConstants.COLUMN_WORKMAIN_WORKROWID +" =? ",selargs);
		}finally{
			baseDao.close();
		}
		return res;
	...
	public HashMap<String, Object> getMeasurementUnit() {
		// TODO Auto-generated method stub
		List<String> lstOfMeasurementUnits = new ArrayList<String>();
		HashMap<String, Object> hmReturning = new HashMap<String, Object>();
		HashMap<Integer, String> hmMUnits = new HashMap<Integer, String>();
		hmMUnits.put(0, "-select-");
		...
		if (cursor != null) {
			while (!cursor.isAfterLast()) {
				...
				hmMUnits.put(cursor.getInt(cursor
						.getColumnIndex(DBConstants.COLUMN_MASTER_MEASUREMENT_MEASUREMENTID)), cursor.getString(cursor
								.getColumnIndex(DBConstants.COLUMN_MASTER_MEASUREMENT_MEASUREMENTDESCRIPTION)));
				cursor.moveToNext();
			}
		}
		hmReturning.put("List", lstOfMeasurementUnits);
		hmReturning.put("hmMUnits", hmMUnits);
		return hmReturning;
	}
	...
	public long updateWorkItemsTable(WorkItems items) {
		...
		long value;
		String[] selArgs = { String.valueOf(items.getWorkItemTypeId()) ,String.valueOf(items.getWorkRowid())};
		...
		try {
			...
			value = baseDao.Update(DBConstants.TABLE_GESCOM_PROJECT_WORKITEMS,
					contentValues,
					DBConstants.COLUMN_WORKITEMS_WORKITEM_TYPE_ID + "=? and "+DBConstants.COLUMN_WORKITEMS_WORK_ROW_ID+"=?",
					selArgs);
			return value;
		} finally {
			baseDao.close();
...
//		values.put(DBConstants.COLUMN_WORKITEMS_WORKITEM_TYPE_ID, items.getWorkItemTypeId());
	...
	public HashMap<String, Integer> getCompleteMaterialData() {
	 ...
	 public MultiValueMap getEstimationDescriptionMultiValueMap(String projrctId,String lineType,String estimationId)
		...
		public void deletefromApprovephotoCoordinates(String Workrowid){
			String[] args = {Workrowid};
			try{
				baseDao.open();
			Cursor cursor = baseDao.RawQuery("Delete  from GESCOM_ApproveCoor_Photos where Workrowid =? ",args);
			}finally{
				baseDao.close();
			}
		}
		...
		public void deletefromApproveCoordinates(String Workrowid){
			String[] args = {Workrowid};
			try{
				baseDao.open();
			Cursor cursor = baseDao.RawQuery("Delete  from GESCOM_Approved_Coordinates where WorkRowId =? ",args);
			}finally{
				baseDao.close();
			}
		}
		public void deletefromworkmain(String Workrowid){
			String[] args = {Workrowid};
			try{
				baseDao.open();
			Cursor cursor = baseDao.RawQuery("Delete  from Workmain where WorkRowid =? ",args);
			}finally{
				baseDao.close();
			}
		}
		public void deletefromworkItems(String Workrowid){
			String[] args = {Workrowid};
			try{
				baseDao.open();
			Cursor cursor = baseDao.RawQuery("Delete  from WorkItems where WorkRowid =? ",args);
			}finally{
				baseDao.close();
			}
		}
		public void deletefromprojectEstimates(String Workrowid){
			String[] args = {Workrowid};
			try{
				baseDao.open();
			Cursor cursor = baseDao.RawQuery("Delete  from GESCOM_Project_Estimates where WorkRowId =? ",args);
			}finally{
				baseDao.close();
			}
		}
		public void deletefromprojectEstimateType(String Workrowid){
			String[] args = {Workrowid};
			try{
				baseDao.open();
			Cursor cursor = baseDao.RawQuery("Delete  from Gescom_project_EstimationType where ProjectId =? ",args);
			}finally{
				baseDao.close();
			}
		}
		public void deleteworkidfromprogreeimages(String workrowid){
			String[] args = {workrowid};
			try{
				baseDao.open();
				Cursor cursor = baseDao.RawQuery("delete from GESCOM_Progress_Images where WorkRowId =? ", args);
				}finally{
					baseDao.close();
				}
		}
		...
		{
			int HtEstCount = 0;
			Cursor cursor = baseDao.RawQuery("Select DISTINCT  Estimationid from GESCOM_Project_Estimates where Workrowid= "+workRowId+" and TypeOfLine=1 and MaterialId=4007", null);
			if(cursor!=null)
			{
		...
		public String isUploadedYes(String workRowId){
			String[] selargs = {workRowId};
			String value= "";
			Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_APPROVE_COOR_PHOTOS, DBConstants.COLUMN_GESCOM_APPROVE_COOR_PHOTOS_WorkrowId + " =? ", selargs);
			if(cursor != null){
				...
				if(cursor.getCount() >= 1){
					value = cursor.getString(cursor
							.getColumnIndex(DBConstants.COLUMN_GESCOM_APPROVE_COOR_PHOTOS_ISUPLOADED));
  ...
  public void updateStatusWorkmain(String workRowId,String status,String date){
	  String[] selargs = {workRowId};
	  ContentValues values = new ContentValues();
	  values.put(DBConstants.COLUMN_WORKMAIN_WORKSTATUSID,status);
	  values.put(DBConstants.COLUMN_WORKMAIN_DATES,date);
	  try{
		  baseDao.open();
		...
		public void updateSpanLength(String spanLength,String workRowID) {
			String[] selargs ={String.valueOf(workRowID)};
			ContentValues values = new ContentValues();
			values.put(DBConstants.COLUMN_WORKMAIN_SPAN_LENGTH,spanLength);
			
			try{
				baseDao.open();
				
				long result=baseDao.Update(DBConstants.TABLE_WORKMAIN, values,DBConstants.COLUMN_WORKMAIN_WORKROWID+"=?", selargs);
				System.out.println(result);
			}finally{
				baseDao.close();
			}	
		}
      public Workmain getWorkmain(String workRowID)
      {
    	  Workmain workmain=new Workmain();
    	  String[] selArg={workRowID};
    	  Cursor cursor = baseDao.Query(DBConstants.TABLE_WORKMAIN, DBConstants.COLUMN_WORKMAIN_WORKROWID+"=?", selArg);
			if (cursor != null) {
				int count = cursor.getCount();
				while (!cursor.isAfterLast()) {
					workmain.setSpanLength(cursor.getString(cursor.getColumnIndex(DBConstants.COLUMN_WORKMAIN_SPAN_LENGTH)));
					workmain.setProjectCategory(cursor.getInt(cursor.getColumnIndex(DBConstants.COLUMN_WORKMAIN_PROJECT_CATEGORY)));
					cursor.moveToNext();
				}
	...
	public void updateMeasurementQuantitiesToEstQuantities(String workRowId) {
		// TODO Auto-generated method stub
		String[] selArgs ={workRowId};
		String query = "update WorkItems set MeasurmentQuantity = TotalUnits, MTotalAmount = TotalAmount where WorkRowid = "+workRowId;
		baseDao.RawQuery(query, null);
	}

	
	public boolean IsestIdone(String workid,int Lineid){
		boolean result = false;
		String[] args = {workid,String.valueOf(Lineid)};
		Cursor cursor = baseDao.Query(DBConstants.TABLE_GESCOM_PROJECT_ESTIMATION_TYPE,DBConstants.COLUMN_GESCOM_PROJECT_ESTIMATIONTYPE_PROJECT_ID + " =? and " + DBConstants.COLUMN_GESCOM_PROJECT_ESTIMATIONTYPE_LINE_ID + " =? ", args);
		if(cursor !=null){
		int count = cursor.getCount();
		if(count >= 1){
			cursor.close();
	...
	public List<Integer> getProjectIdforSavecordinate(String staus,String user) {
		String[] selArgs = { staus,user };
		ArrayList<Integer> workInfo = new ArrayList<Integer>();
		...
		if (cursor != null) {
			...
			while (!cursor.isAfterLast()) {
				int eoid = cursor.getInt(0);
				String eoid1 = cursor.getString(1);
				workInfo.add(cursor.getInt(cursor.getColumnIndex(DBConstants.COLUMN_WORKMAIN_WORKROWID)));
				//workInfo.add(cursor.getString(cursor.getColumnIndex(DBConstants.COLUMN_WORKMAIN_WORKROWID)));
				cursor.moveToNext();
			}
	...
	public void updateProgressImagesSyncyes(String workRowId, ApprovedPhotoCoordinates photoCoordinates) {
		// TODO Auto-generated method stub
		
		String[] selargs = {workRowId,photoCoordinates.getFileName()};
		
		contentValues = new ContentValues();
		contentValues.put(DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_ISIMAGEUPLOADED, "Y");
		try{
			baseDao.open();
			baseDao.Update(DBConstants.TABLE_GESCOM_PROGRESS_IMAGES, contentValues, DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_WORKROWID+"=? and "+DBConstants.COLUMN_GESCOM_PROGRESS_IMAGES_FILENAME+"=?", selargs);
		}finally
		{
			baseDao.close();
		}
	...
	public int getSpanLength(String workRowId) {
		// TODO Auto-generated method stub
		...
		if (cursor != null) {
			if(cursor.getCount()>0){
				return cursor.getInt(cursor.getColumnIndex(DBConstants.COLUMN_WORKMAIN_SPAN_LENGTH));
			}
